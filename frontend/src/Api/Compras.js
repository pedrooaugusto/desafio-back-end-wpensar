...
export async function list() {
	...
	return hasToken(async (err, token, code) => {
		...
		const res = await fetch(LOCAL_HOST+'/api/compras/?format=json', {
...
export async function remove(id){
	return hasToken(async (err, token, code) => {
		if(err)
			return {error: true, code: 401};
		const res = await fetch(LOCAL_HOST+'/api/compras/'+id+'/?format=json', {
			headers: makeHeaders(token),
			method: 'delete'
		});
		return checkToken(res, async (err, obj) => {
...
export async function edit(id, obj){
	return hasToken(async (err, token, code) => {
		if(err)
			return {error: true, code: 401};
		const res = await fetch(LOCAL_HOST+'/api/compras/'+id+'/', {
			headers: makeHeaders(token),
			method: 'put',
